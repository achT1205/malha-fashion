    ...
    <div class="relative w-full max-w-screen-lg overflow-hidden">
      ...
      >
        <router-link
          ...
          class="group block flex-none w-full"
        >
          <div
            aria-hidden="true"
...
export default {
  ...
  methods: {
    nextImage() {
      this.currentIndex = (this.currentIndex + 1) % this.collections.length
    },
    prevImage() {
      this.currentIndex =
        (this.currentIndex + this.collections.length - 1) % this.collections.length
    },
    jumpTo(index) {
